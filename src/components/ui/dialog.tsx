...
interface DialogBackdropProps extends React.ComponentPropsWithoutRef<typeof DialogPrimitive.Backdrop> {}
...
const DialogBackdrop = React.forwardRef<HTMLDivElement, DialogBackdropProps>(
  ({ className, ...props }, ref) => (
    <DialogPrimitive.Backdrop
      ref={ref}
      className={cn(
        "fixed inset-0 z-50 bg-black/60 backdrop-blur-xs transition-opacity duration-300 data-[state=open]:opacity-100 data-[state=closed]:opacity-0",
        className
      )}
      {...props}
    />
...
export {
  Dialog,
  DialogPortal,
  DialogOverlay, // compatibility alias
  DialogClose,
  DialogTrigger,
  DialogContent,
  DialogHeader,
